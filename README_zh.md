# Claude Code Router Fixed (多用户环境修复版)

> **致谢**: 本项目基于 [musistudio/claude-code-router](https://github.com/musistudio/claude-code-router/tree/main) 修改而来，修复了多用户服务器环境下的兼容性问题。感谢原作者的优秀工作！
...
### 修复1: 端口冲突
**问题描述：** 在多用户服务器环境中，当多个用户同时运行 Claude Code Router 时，默认端口 3456 会发生冲突，导致后启动的用户无法成功启动服务。

**修复过程：** 
- 增加了 `PORT` 配置项，允许用户在 `config.json` 中自定义端口
- 新增自动端口切换功能：通过 `AUTO_PORT_SWITCH` 配置项（默认启用），当指定端口被占用时，系统会自动查找下一个可用端口，并将新端口保存到配置文件
- 实现了端口可用性检测机制，在服务启动前验证端口状态
- 提供清晰的端口切换提示信息，方便用户了解实际使用的端口

### 修复2: 权限冲突（claude-code-reference-count.txt）
**问题描述：** 多个用户在同一服务器上运行时，`claude-code-reference-count.txt` 文件的读写权限冲突会导致服务启动失败或异常终止。

**修复过程：**
- 修改了引用计数文件的访问逻辑，添加了错误处理机制
- 实现了用户级别的隔离，确保每个用户使用独立的配置目录
- 优化了文件锁定机制，避免并发访问冲突
...
然后，安装 Claude Code Router Fixed（多用户环境修复版）：
...
- **`PORT`** (可选): 您可以设置服务的端口号。默认值为 `3456`。
- **`AUTO_PORT_SWITCH`** (可选): 启用自动端口切换功能。当设置为 `true`（默认值）时，如果指定的端口已被占用，系统会自动查找并使用下一个可用端口（最多尝试 50 个端口）。自动切换的端口会被保存到配置文件中。如果设置为 `false`，当端口被占用时服务将无法启动。例如：`"AUTO_PORT_SWITCH": true`。
...
如果您觉得这个项目有帮助，请考虑赞助[原项目开发者](https://github.com/musistudio/claude-code-router/tree/main)。非常感谢您的支持！
