...
import { initConfig, initDir, cleanupLogFiles, writeConfigFile } from "./utils";
import { createServer } from "./server";
import { router } from "./utils/router";
import { apiKeyAuth } from "./middleware/auth";
import { getAvailablePort } from "./utils/portCheck";
import {
  cleanupPidFile,
  isServiceRunning,
  savePid,
...
async function run(options: RunOptions = {}) {
  ...
  // 配置是否启用自动端口切换（默认启用）
  const autoPortSwitch = config.AUTO_PORT_SWITCH !== false;
  ...
  // 检测端口可用性并自动切换（如果启用）
  const portCheckResult = await getAvailablePort(servicePort, autoPortSwitch);
  
  if (portCheckResult.port === -1) {
    console.error(`❌ ${portCheckResult.message}`);
    console.error("Please change the PORT in ~/.claude-code-router/config.json or enable AUTO_PORT_SWITCH.");
    cleanupPidFile();
    process.exit(1);
  }
  
  if (portCheckResult.switched) {
    console.warn(`⚠️  ${portCheckResult.message}`);
    servicePort = portCheckResult.port;
    
    // 更新配置文件中的端口
    try {
      config.PORT = servicePort;
      await writeConfigFile(config);
      console.log(`✅ Configuration updated: PORT set to ${servicePort}`);
    } catch (error) {
      console.warn(`⚠️  Failed to update config file, but service will use port ${servicePort}`);
    }
  } else {
    console.log(`✅ Using port ${servicePort}`);
  }
